...
async function run() {
  try {
    ...
    const availableAppointmentsCollection = database.collection(
      "availableAppointments"
    );

    //get available appointments
    app.get("/availableAppointments", async (req, res) => {
      const cursor = await availableAppointmentsCollection.find({}).toArray();
      res.json(cursor);
    });
    ...
    app.post("/availableAppointments", async (req, res) => {
      const service = req.body;
      const result = await availableAppointmentsCollection.insertOne(service);
      res.json(result);
    });
